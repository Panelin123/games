...
function updateTotalScore(scoreToAdd) {
    ...
    if (!currentUser || scoreToAdd <= 0) return; // Só adiciona score positivo se usuário logado
    
    try {
        const scores = JSON.parse(localStorage.getItem(SCORE_KEY_TOTAL) || '[]');
        let userEntry = scores.find(s => s.user === currentUser);

        if (userEntry) {
            userEntry.score += scoreToAdd;
        } else {
            scores.push({ user: currentUser, score: scoreToAdd });
        }

        localStorage.setItem(SCORE_KEY_TOTAL, JSON.stringify(scores));
        console.log(`Pontuação de ${scoreToAdd} adicionada ao total de ${currentUser}. Novo total: ${userEntry ? userEntry.score : scoreToAdd}`);
...
document.addEventListener('DOMContentLoaded', function() {
    const currentUser = getCurrentUser();
    if (!currentUser) {
        console.log('⚠️ Usuário não logado - pontuações não serão salvas no placar global');
    }
});
...
function gameOver() {
    ...
    if (currentUser) {
        ...
        finalMessage += `\n🏆 ${finalScore} pontos adicionados ao seu High Score Global!`;
    } else {
        finalMessage += `\n⚠️ Faça login no Menu para pontuar no High Score Global!`;
    }
